...
class DataLoader:
    ...
    def __init__(self) -> None:
        ...
        self.surnames = load_csv(
            res_path + "/family_names/family_names_spanish.txt")
...
class Person:
    ...
    def __init__(self, student=True) -> None:
        self._name = random.choice(list(dataLoader.names))
        self._first_surname = random.choice(list(dataLoader.surnames))
        self._second_surname = random.choice(list(dataLoader.surnames))
        self.curriculum = []

        if student:
            self.years = dataLoader.academic_year_tags
    ...
    def populate_courses(self):
        ...
        if len(self.curriculum) == 0:
            for year in self.years:
                course = []
                subjects = list(dataLoader.subjects)
                ...
                for subject in subjects:
                    ...
                    verbose_name = random.choice(list(dataLoader.subjects[subject]))
    ...
    def get_replacements(self, verbose_level=0):
        ...
        for year in self.curriculum:
            ...
            for subject in year:
                ...
                replacements[f"replace_grade_{i}_{j}"] = return_verbose_grade(subject[key]['grade'], verbose_level)
                j += 1
            i += 1
